...
public:
    ...
    void organizePapers2(double t1);
    void organizePapers3(double t1);

    void randomState (int seed);
    
    double fast_score(double curr_state_goodness, int t1,int p1,int k1, int t2,int p2, int k2,int c_trade_off);
    double sim_in_a_session(int t,int p,int k,int tdiff,int pdiff, int kdiff, bool sim);
    /**
